...
import { X, Users, Loader2, Save, Trash2, Shield } from "lucide-react";
...
export function ManageTeamModal({
  ...
  const handleRemoveEmployee = async (employeeId: string, name: string) => {
    if (
      !confirm(
        `Are you sure you want to completely remove ${name} from your team? This action cannot be undone.`,
      )
    )
      return;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Update titles, roles, or remove team members.
              </p>
            </div>
          </div>
          <button
          ...
          {!employees || employees.length === 0 ? (
            ...
            <div className="space-y-4">
              {employees.map((emp) => (
                <div
                  key={emp.id}
                  className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-muted/40 rounded-xl border border-border/50 gap-4"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-medium truncate">{emp.full_name}</p>
                      {emp.role === "manager" && (
                        <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-md bg-brand-soft text-brand">
